...
if page == pages[0] :
    ...
    st.image (r"C:\Users\MOUK\OneDrive\Bureau\appli_streamlit\image\image.png",
         caption="Santé",
    ...
    st.image (r"C:\Users\MOUK\OneDrive\Bureau\appli_streamlit\image\tléchargement.png",
         caption="Santé",
         use_container_width = True)
    ...
    df = pd.read_csv(r"C:\Users\MOUK\OneDrive\Bureau\appli_streamlit\Data\incidence-of-malaria.csv")
    st.dataframe(df)
    st.write ("Dimensions du tableau :", df.shape)
    
    st.write ("Data sur la Mortalité maternelle")
    dt = pd.read_csv(r"C:\Users\MOUK\OneDrive\Bureau\appli_streamlit\Data\mat-mortality.csv")
    st.dataframe(dt)
...
# --- Graph par pays et par maladie sur les années ---
    ...
    figs_1 = plt.figure (figsize = (10,14), dpi = 100)
    sns.boxplot (df['Incidence of malaria (per 1,000 population at risk)'])
    plt.title (' Boxplot de : Incidence of malaria (per 1,000 population at risk)')
    plt.savefig('boxplot_paludisme.png') # Save the boxplot
    plt.show ()
    st.pyplot (figs_1)

    figs_2 = plt.figure (figsize = (10,10), dpi = 100)
    sns.histplot (df['Incidence of malaria (per 1,000 population at risk)'])
    plt.title (' Histplot de : Incidence of malaria (per 1,000 population at risk)')
    plt.savefig('histplot_paludisme.png') # Save the histplot
    plt.show ()
    st.pyplot (figs_2)

    figs_3 = plt.figure (figsize = (12,10), dpi = 100)
    dt = pd.read_csv(r"C:\Users\MOUK\OneDrive\Bureau\appli_streamlit\Data\mat-mortality.csv")
    sns.histplot (dt["World region according to OWID"])
    plt.title ('Barplot de : World region according to OWID')
    plt.savefig('barplot_maternal_region.png') # Save the barplot
    plt.show ()
    st.pyplot (figs_3)
...
# --- Carte choroplèthe dynamique ---
    ...
    figbi_1 = plt.figure (figsize=(10,5))
    plt.plot(df1["Year"], df1["Incidence of malaria (per 1,000 population at risk)"])
    plt.title("Évolution mondiale du paludisme de 2000 à 2024")
    plt.savefig('timeseries_paludisme.png') # Save the time series plot
    plt.xlabel("Année")
    ...
    figbi_2 = plt.figure(figsize=(12, 6))
    sns.lineplot(data = dt1, x='Year', y='Maternal mortality ratio')
    plt.title('Évolution Mondiale du Ratio de Mortalité Maternelle par An')
    plt.xlabel('Année')
    plt.ylabel('Ratio de Mortalité Maternelle')
    plt.grid(True)
    plt.savefig('timeseries_maternal_mortality.png') # Save the time series plot
    plt.show()
    st.pyplot (figbi_2)
    ...
    st.image (r"C:\Users\MOUK\OneDrive\Bureau\appli_streamlit\image\ChatGPT Image 30 juin 2026, 22_20_34.png",
         caption="Santé",
         use_container_width = True)
